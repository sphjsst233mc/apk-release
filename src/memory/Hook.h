...
namespace memory {
    ...
    template <typename T, std::enable_if_t<
        (std::is_member_function_pointer<T>::value &&
            sizeof(T) == sizeof(FuncPtr) + sizeof(ptrdiff_t)), int> = 0>
    constexpr FuncPtr toFuncPtr(T t) {
        union {
            struct {
                FuncPtr   fp;
    ...
    template <typename T>
    using FuncPtrType = std::integral_constant<bool,
        std::is_function<std::remove_pointer_t<T>>::value ||
        std::is_member_function_pointer<T>::value>;

    template <typename T, typename = std::enable_if_t<
        FuncPtrType<T>::value || std::is_same<T, uintptr_t>::value>>
    ...
    template <typename T>
    constexpr FuncPtr resolveIdentifier(std::initializer_list<const char*> identifiers) {
...
#define HOOK_IMPL(REGISTER, FUNC_PTR, STATIC, CALL, DEF_TYPE, TYPE, PRIORITY, IDENTIFIER, RET_TYPE, ...)               \
    struct DEF_TYPE TYPE {                                                                                             \
        ...
        using OriginFuncType =                                                                                         \
            ::memory::AddConstAtMemberFunIfOriginIs<RET_TYPE FUNC_PTR(__VA_ARGS__), decltype(IDENTIFIER_STORAGE)>;     \
                                                                                                                       \
